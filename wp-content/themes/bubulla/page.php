...
if ( function_exists( 'FW' ) ) {
	...
	$bubulla_current_scheme_db = fw_get_db_post_option( $wp_query->get_queried_object_id(), 'color-scheme' );
	add_filter ('bubulla_current_scheme', function() {
		global $wp_query;	

		$bubulla_current_scheme_db = fw_get_db_post_option( $wp_query->get_queried_object_id(), 'color-scheme' );
		if ($bubulla_current_scheme_db == 'default') $bubulla_current_scheme_db = 1; 
		return $bubulla_current_scheme_db; 
	} );
...
if ( !bubulla_check_active_sidebar() OR bubulla_is_wc('cart') OR bubulla_is_wc('checkout') ) {
