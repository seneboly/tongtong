 ...
 */
if ( !function_exists( 'bubulla_single_comment' ) ) {

	function bubulla_single_comment( $comment, $args, $depth ) {

		$GLOBALS['comment'] = $comment;
		switch ( $comment->comment_type ) {
			case 'pingback' :
				?>
				<li class="trackback"><?php esc_html_e( 'Trackback:', 'bubulla' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( esc_html__( 'Edit', 'bubulla' ), '<span class="edit-link">', '<span>' ); ?>
				<?php
				break;
			case 'trackback' :
				?>
				<li class="pingback"><?php esc_html_e( 'Pingback:', 'bubulla' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( esc_html__( 'Edit', 'bubulla' ), '<span class="edit-link">', '<span>' ); ?>
				<?php
				break;
			default :
					...
					<article id="comment-<?php comment_ID(); ?>" class="comment-body comment-single">
						...
						<div class="comment-content">
							<div class="comment-info">
	                            <span class="comment-date-time">
	                            	<span class="comment-date">
	                            		<span class="date-value">
			                            	...
			                            	<?php echo esc_html__( 'at', 'bubulla' ); ?>
								...
								<div class="comment_not_approved"><?php esc_html_e( 'Your comment is awaiting moderation.', 'bubulla' ); ?></div>
								<?php } ?>	                            
							</div>
 ...
 */
if ( !function_exists( 'bubulla_the_before_footer' ) ) {
	
	function bubulla_the_before_footer() {
 ...
 */
if ( !function_exists( 'bubulla_the_subscribe_block' ) ) {
	
	function bubulla_the_subscribe_block() {
 ...
 */
if ( !function_exists( 'bubulla_the_topbar_block' ) ) {

	function bubulla_the_topbar_block( $navbar_layout ) {
 ...
 */
if ( !function_exists( 'bubulla_the_footer_logo_section' ) ) {

	function bubulla_the_footer_logo_section() {
        ...
        $bubulla_logo = fw_get_db_settings_option( 'logo' );
		?>
		<section id="ltx-logo-footer">
			...
			<div class="container">
	            <?php
					...
					if ( !empty($bubulla_logo) ) {

						echo '<span class="logo-footer">'.wp_get_attachment_image( $bubulla_logo['attachment_id'], 'full' ).'</span>';
					}

					bubulla_the_social_footer();
	            ?>
			</div>
		</section>
		<?php		
 ...
 */
if ( !function_exists( 'bubulla_the_footer_icons' ) ) {

	function bubulla_the_footer_icons() {
 ...
 * Prints Footer widgets block
 */
if ( !function_exists( 'bubulla_the_footer_widgets' ) ) {

	function bubulla_the_footer_widgets( $layout = null ) {
	    ...
	    if ( $bubulla_footer_cols['num'] > 0 ): ?>
		<section id="ltx-widgets-footer" class="<?php echo esc_attr($footer_class); ?>" >
			<div class="container">
				<div class="row row-center-x">
	                <?php
	                for ($x = 1; $x <= 4; $x++): ?>
	                    <?php if ( !isset($bubulla_footer_cols['hidden'][ $x ]) && is_active_sidebar( 'footer-' . $x ) ): ?>
						<div class="<?php echo esc_attr( $bubulla_footer_cols['classes'][$x] ).' '.esc_attr( $bubulla_footer_cols['hidden_mobile'][$x] ).' '.esc_attr( $bubulla_footer_cols['hidden_md'][$x] ); ?> clearfix">    
...
if ( !function_exists( 'bubulla_get_the_logo' ) ) {
	...
	function bubulla_get_the_logo( $layout = null ) {
		...
		if ( function_exists( 'FW' ) ) {
			...
			$current_scheme =  apply_filters ('bubulla_current_scheme', array());
	...
	function bubulla_the_logo( $layout = null ) {

		echo bubulla_get_the_logo($layout);
	}
}
 ...
 * Display H1 header
*/
if ( !function_exists( 'bubulla_the_h1' ) ) {

	function bubulla_the_h1() {

		$title = bubulla_get_the_h1();
 ...
 */
if ( !function_exists( 'bubulla_the_pageloader_overlay' ) ) {

	function bubulla_the_pageloader_overlay() {
		...
		if ( function_exists( 'FW' ) ) {
			...
			if ( !empty($pace) AND ((!empty($pace['loader']) AND $pace['loader'] != 'disabled') OR 
			   ( !empty($pace) AND $pace['loader'] != 'disabled') ) ) {
				...
				echo '<div id="ltx-preloader" data-loader="'.esc_html__( 'Loading...', 'bubulla' ).'"></div>';
			}
		}
	}
}
 ...
 */
if ( !function_exists( 'bubulla_the_copyrights' ) ) {

	function bubulla_the_copyrights() {

		if ( function_exists( 'FW' ) ) {

			$bubulla_copyrights = fw_get_db_settings_option( 'copyrights' );

			if ( !empty($bubulla_copyrights) ) {

				echo wp_kses( $bubulla_copyrights, 'header' );	
			}
				else {
				
				echo '<p>'. esc_html__( 'Like-themes &copy; All Rights Reserved - 2019', 'bubulla' ) .'</p>';
			}
		}
			else {
 ...
 * Footer overlay
 */
if ( !function_exists( 'bubulla_the_footer_overlay' ) ) {

	function bubulla_the_footer_overlay() {
 ...
 */
if ( !function_exists( 'bubulla_the_copyrights_section' ) ) {

	function bubulla_the_copyrights_section() {
 ...
 */
if ( !function_exists( 'bubulla_the_go_top' ) ) {

	function bubulla_the_go_top() {
            	...
            	echo '<a href="#" class="ltx-go-top '.esc_attr(implode(' ', $class)).'">';
            		...
            		if ( !empty($go_top_img) ) {
	                	...
	                	if ( empty( $go_top_text ) ) {
	                		...
	                		$go_top_text = esc_html__( 'Go Top', 'bubulla' );
 ...
 * Blog related posts
 */
if ( !function_exists( 'bubulla_related_posts' ) ) {

	function bubulla_related_posts($id) {
		...
		if ( !empty( $tags ) ) {
			...
			if ( $my_query->have_posts() ) {
				...
				set_query_var( 'bubulla_featured_disabled', true );
				...
				echo '<div class="heading has-subheader text-align-center subcolor-main">';
					echo sprintf(
 					...
 					esc_html__( 'Related ', 'bubulla' ),
 					esc_html__( 'posts', 'bubulla' )
 				);
				echo '</div>';
			...
			wp_reset_postdata();
			set_query_var( 'bubulla_featured_disabled', false );
		}
	}
}
 ...
 */
if ( !function_exists( 'bubulla_author_bio' ) ) {

	function bubulla_author_bio( ) {
		...
		if ( is_single() && isset( $post->post_author ) ) {
			...
			$author_details .= '<p class="author-links">';
				...
				if ( ! empty( $user_website ) ) {
					...
					$author_details .= '<a href="' . esc_url( $user_website ) .'" class="btn btn-main color-hover-white btn-xs" target="_blank" rel="nofollow">'. esc_html__( 'Website', 'bubulla' ) . '</a>';
				}

				$author_details .= '<a href="'. esc_url( $user_posts ) .'" class="btn btn-xs btn-main">'. esc_html__( 'All posts', 'bubulla' ) . '</a>';  
			...
			$content = '<section class="ltx-author-bio">';
				...
				$content .= '<div class="author-info">';
					if ( ! empty( $display_name ) ) {
						...
						$content .= '<span><p class="author-name">'. esc_html__( 'Author', 'bubulla' ) . '</p><h5>'. $display_name . '</h5></span>';
					}				
...
if ( !function_exists( 'bubulla_get_the_post_headline' ) ) {
	...
	function bubulla_get_the_post_headline( $headline = 'inline' ) {

		echo '<div class="ltx-post-headline">';

		    if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {

		    	echo '<a href="'. esc_url( get_the_permalink() ) .'" class="ltx-comments"><span>';

		    	echo get_comments_number_text( 
		    		esc_html__('0 comments', 'bubulla'), esc_html__('1 Comment', 'bubulla'), esc_html__('% Comments', 'bubulla')
		    	);
			...
			if ( function_exists( 'pvc_post_views' ) ) {
				...
				echo '<span class="ltx-icon-fav"><span>' . esc_html( $count ) . ' ' . _n( 'View', 'Views', (int)($count), 'bubulla' ) .'</span></span>';
			}

		echo '</div>';
	}

	function bubulla_get_the_post_headline_left() {

		echo '<div class="ltx-post-headline">';

		echo '<div class="ltx-user">'.get_avatar( get_the_author_meta('user_email'), 50 ).'<span class="info">'. esc_html__( 'by', 'bubulla' ) . ' ' .get_the_author_link().'</span></div>';		
...
if ( !function_exists( 'bubulla_the_blog_date' ) ) {
	...
	function bubulla_the_blog_date( $args = array() ) {
...
if ( !function_exists( 'bubulla_get_the_cats_archive' ) ) {
	...
	function bubulla_get_the_cats_archive() {

		if ( in_array( 'category', get_object_taxonomies( get_post_type() ) ) && bubulla_categorized_blog() ) {
 ...
 * 
 */
if ( !function_exists( 'bubulla_the_post_info' ) ) {

	function bubulla_the_post_info( $showText = true, $wrapper = true ) {
		...
		if ( function_exists( 'pvc_post_views' ) ) {
			...
			if ( $showText) {
				...
				echo '<li class="ltx-icon-fav">
					<i>' . esc_html( $count ) . ' ' . _n( 'View', 'Views', (int)($count), 'bubulla' ) .'</i>
				</li>';
			}
				else {
 ...
 * 
 */
if ( !function_exists( 'bubulla_get_the_navbar_icons' ) ) {

	function bubulla_get_the_navbar_icons( $layout = null, $mobile = false ) {
				...
				if ( $item['type']['type_radio'] == 'search') {
					...
					<li class="ltx-fa-icon ltx-nav-search  '.esc_attr($li_class).'">
						<div class="top-search" data-base-href="'. esc_url( home_url( '/' ) ) .'">
							...
							'.$close.'
							<input placeholder="'.esc_attr__( 'Search', 'bubulla' ).'" value="'. esc_attr( get_search_query() ) .'" type="text">
						</div>
					</li>';
				}
					...
					$items .= '
						<li class="ltx-fa-icon ltx-nav-cart '.esc_attr($li_class).'">
							<div class="cart-navbar">
								<a href="'. wc_get_cart_url() .'" class="ltx-cart cart shop_table" title="'. esc_attr__( 'View your shopping cart', 'bubulla' ). '">';
		...
		if ( !empty($items) ) {
			...
			if ( empty( $mobile ) ) {
				...
				return '<div class="ltx-navbar-icons"><ul>'.$items.'</ul></div>';
			}
				else {
	...
	function bubulla_the_navbar_icons() {

		echo bubulla_get_the_navbar_icons();
	}
}
 ...
 */
if ( !function_exists( 'bubulla_the_breadcrumbs' ) ) {

	function bubulla_the_breadcrumbs() {
 ...
 * Tagline in header
 */
if ( !function_exists( 'bubulla_the_tagline_header' ) ) {

	function bubulla_the_tagline_header() {
 ...
 */
if ( !function_exists( 'bubulla_the_social_header' ) ) {

	function bubulla_the_social_header() {

		if ( function_exists( 'FW' ) ) {
			
			$bubulla_social = fw_get_db_settings_option( 'header-social' );
			$bubulla_social_text = fw_get_db_settings_option( 'header-social-text' );

			if ( $bubulla_social == 'enabled' ) {
 ...
 */
if ( !function_exists( 'bubulla_the_social_footer' ) ) {

	function bubulla_the_social_footer() {
 ...
 */
if ( !function_exists( 'bubulla_the_navbar_social' ) ) {

	function bubulla_the_navbar_social() {
